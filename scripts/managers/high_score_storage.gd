...
func _migrate_from_v1_0(old_data: Dictionary) -> Array[Dictionary]:
	...
	var scores = old_data.scores as Array[Dictionary]
	var enhanced_scores: Array[Dictionary] = []
...
func _migrate_unknown_format(data) -> Array[Dictionary]:
	...
	if data is Array:
		...
		if data.has("scores") and data.scores is Array:
			scores = data.scores as Array[Dictionary]
		elif data.has("high_scores") and data.high_scores is Array:
			scores = data.high_scores as Array[Dictionary]
		elif data.has("leaderboard") and data.leaderboard is Array:
			scores = data.leaderboard as Array[Dictionary]
...
func attempt_save_file_recovery() -> Array[Dictionary]:
	...
	var scores = _load_from_file(found_path)
	if scores != null and scores.size() > 0:
		# Re-save to the updated path for consistency
		save_high_scores(scores)
	return scores if scores != null else [] as Array[Dictionary]
...
func _load_from_file(file_path: String) -> Array[Dictionary]:
	...
	var file = FileAccess.open(file_path, FileAccess.READ)
	if not file:
		return [] as Array[Dictionary]  # Changed: Return empty array for file open failure (consistent with other failures)
	...
	if detected_version != "unknown":
		...
		if migrated_scores.size() > 0:
			...
			return migrated_scores  # Unchanged: Return migrated data
		else:
			if debug_logging:
...
func _calculate_checksum(scores: Array[Dictionary]) -> String:
	...
	var content = ""
	for score in scores:
		# Changed: Use get() with defaults to include all entries, avoiding skips
		content += str(score.get("name", "")) + str(score.get("score", 0))
...
func _get_migration_history_from_scores(scores: Array[Dictionary]) -> Array[String]:
	...
	var migration_sources: Array[String] = []
