...
func _ready():
	...
	# Connect egg area signal
	egg_area.connect("body_entered", _on_egg_area_body_entered)
...
func process_walking(delta):
	...
	if position.x < -enemy_sprite.texture.get_width():
		position.x = viewport_size.x + enemy_sprite.texture.get_width()
	elif position.x > viewport_size.x + enemy_sprite.texture.get_width():
		position.x = -enemy_sprite.texture.get_width()
...
func _on_egg_area_body_entered(body):
	...
	print("Body entered egg area: ", body.name)
	if current_state == State.EGG and body.is_in_group("players"):
		print("Egg collected by player!")
		collect_egg()
...
func _on_hatch_timer_timeout():
	current_state = State.FLYING
	enemy_sprite.modulate = Color(1, 1, 1)  # Reset color
	# Change sprite back to enemy (would use animation in full implementation)
